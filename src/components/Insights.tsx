...
export function Insights() {
  ...
  const monthlyData = (() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      months.push({ label: d.toLocaleDateString('en-US', { month: 'short' }), total, key });
    }
    return months;
  })();
  ...
  const categoryData = (() => {
    ...
    receipts.forEach((r) => {
      map.set(r.store_category, (map.get(r.store_category) || 0) + Number(r.total_amount));
    });
  ...
  const topStores = (() => {
    ...
    receipts.forEach((r) => {
      const existing = map.get(r.store_name) || { count: 0, total: 0 };
      map.set(r.store_name, {
        count: existing.count + 1,
        total: existing.total + Number(r.total_amount),
      });
    });
    return Array.from(map.entries())
  ...
  if (receipts.length === 0) {
    return (
      <div className="min-h-screen bg-ink-50 pb-28 flex flex-col items-center justify-center px-8">
        ...
        <p className="font-semibold text-ink-900 mb-1">No insights yet</p>
        <p className="text-ink-400 text-sm text-center">Scan some receipts to see your spending insights.</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-ink-50 pb-28">
      <div className="px-6 pt-12 pb-4">
        <h1 className="text-2xl font-extrabold text-ink-900 tracking-tight mb-1">Your Spending</h1>
        <p className="text-ink-500 text-sm">Insights from your purchase history</p>
      </div>
      ...
      <div className="px-6 space-y-4">
        ...
        <div className="bg-white rounded-2xl p-5 border border-ink-100 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-ink-900 text-sm">Monthly Spending</h3>
          </div>
                ...
                <div className="w-full flex-1 flex items-end">
                  <div
                    className="w-full rounded-t-lg bg-gradient-to-t from-primary-500 to-primary-400
                      transition-all duration-500 hover:from-primary-600 hover:to-primary-500
                      min-h-[4px] relative group"
        ...
        <div className="bg-white rounded-2xl p-5 border border-ink-100 shadow-sm">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-semibold text-ink-900 text-sm">Spending Trend</h3>
          </div>
          <p className="text-sm text-ink-600">
            You spent{' '}
            <span className={`font-bold ${trendPct > 0 ? 'text-red-500' : 'text-emerald-500'}`}>
              {Math.abs(trendPct).toFixed(0)}% {trendPct > 0 ? 'more' : 'less'}
            </span>{' '}
        ...
        <div className="bg-white rounded-2xl p-5 border border-ink-100 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-ink-900 text-sm">Top Categories</h3>
          </div>
              ...
              return (
                <div key={category}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold text-ink-900">{formatCurrency(total)}</span>
                      <span className="text-xs text-ink-400 w-10 text-right">{pct.toFixed(0)}%</span>
                    </div>
                  </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-ink-100 shadow-sm">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-ink-900 text-sm">Top Stores</h3>
          </div>
          <div className="space-y-3">
            {topStores.map((store, i) => (
              <div key={store.store} className="flex items-center gap-3">
                <span className="w-6 h-6 rounded-full bg-ink-100 flex items-center justify-center text-xs font-bold text-ink-600 shrink-0">
                  {i + 1}
                </span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-ink-900 truncate">{store.store}</p>
                  <p className="text-xs text-ink-400">{store.count} receipt{store.count !== 1 ? 's' : ''}</p>
                </div>
                <p className="text-sm font-bold text-ink-900 shrink-0">{formatCurrency(store.total)}</p>
              </div>
            ))}
          </div>
        </div>
