...
const exampleQueries = [
  ...
  'When did I buy batteries?',
  'Show electronics purchases',
  'Coffee last month',
];
...
export function SearchScreen({ onReceiptClick }: SearchProps) {
  ...
  const performSearch = (q: string) => {
    ...
    // Smart search: match store, category, date terms, item names, amounts
    const filtered = allReceipts.filter((r) => {
      ...
      // Date match (month names, "last month", "today", "yesterday")
      const dateStr = new Date(r.receipt_date + 'T00:00:00').toLocaleDateString('en-US', {
        month: 'long',
        day: 'numeric',
        year: 'numeric',
      }).toLowerCase();
      ...
      // Amount match (e.g., "under 50", "over 100")
      ...
      // Generic word match against store + category
  ...
  return (
    <div className="min-h-screen bg-ink-50 pb-28">
      ...
      <div className="px-6 pt-12 pb-4">
        <h1 className="text-2xl font-extrabold text-ink-900 tracking-tight mb-1">Search Your Vault</h1>
        <p className="text-ink-500 text-sm">Find any receipt instantly</p>
      </div>
        ...
        <div className="relative">
          ...
          {query && (
            <button
              ...
              className="absolute right-4 top-1/2 -translate-y-1/2 w-6 h-6 rounded-full bg-ink-100 flex items-center justify-center text-ink-500 hover:bg-ink-200 transition-colors"
            >
      ...
      <div className="px-6 mb-6">
        <div className="flex gap-2 overflow-x-auto no-scrollbar">
          {filterChips.map((chip) => (
            <button
              key={chip.label}
              className="shrink-0 flex items-center gap-1.5 px-3.5 h-9 rounded-full bg-white border border-ink-200
                text-ink-600 text-xs font-semibold hover:border-primary-400 hover:text-primary-600 transition-colors"
            >
        ...
        <div className="px-6">
          ...
          <div className="bg-gradient-to-br from-primary-50 to-white rounded-2xl p-5 border border-primary-100 mb-6">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-bold text-primary-800">Try asking</span>
            </div>
            <div className="space-y-2">
              {exampleQueries.map((ex) => (
                <button
                  key={ex}
                  onClick={() => setQuery(ex)}
                  className="block w-full text-left text-sm text-ink-700 font-medium px-3 py-2.5 rounded-xl hover:bg-white transition-colors"
                >
                  "{ex}"
                </button>
              ))}
          ...
          {/* Recent searches */}
          <div>
            <h3 className="text-sm font-bold text-ink-900 mb-3">Recent Searches</h3>
            <div className="flex flex-wrap gap-2">
              {recentSearchChips.map((chip) => (
                <button
                  key={chip}
                  onClick={() => setQuery(chip)}
                  className="px-4 h-9 rounded-full bg-white border border-ink-200 text-ink-600 text-sm font-medium
        ...
        <div className="px-6">
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <p className="font-semibold text-ink-900 mb-1">Nothing found</p>
              <p className="text-ink-400 text-sm">Try searching for a store, product, or date.</p>
            </div>
          ) : (
            <>
              <p className="text-sm text-ink-400 font-medium mb-3">
                {results.length} result{results.length !== 1 ? 's' : ''} for "{query}"
              </p>
              <div className="space-y-3">
                {results.map((receipt) => (
                  <SearchResultCard key={receipt.id} receipt={receipt} onClick={() => onReceiptClick(receipt)} />
                ))}
              </div>
            </>
          )}
    ...
    >
      ...
      <div className="flex-1 min-w-0 text-left">
        ...
        <p className="text-ink-400 text-xs">
          {receipt.store_category} · {formatDate(receipt.receipt_date)}
        </p>
      </div>
